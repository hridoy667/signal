...
/** Coerce Prisma $queryRaw row shapes (e.g. bigint) for React state. */
function normalizeFeedPost(row: FeedPost): FeedPost {
  return {
    ...row,
    upvoats: Math.max(0, num(row.upvoats)),
    downvoats: Math.max(0, num(row.downvoats)),
    neutralvoats: Math.max(0, num(row.neutralvoats)),
    comment_count: Math.max(0, num(row.comment_count)),
    userVote: (row.userVote as VoteType | null) ?? null,
  };
}
...
export function FeedClient() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const raw = (res.data as FeedPost[]) ?? [];
      setPosts(raw.map(normalizeFeedPost));
    } catch (e) {
      const msg =
