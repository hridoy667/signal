...
import { createComment, getCommentsByPost } from "@/services/comments.service";
import { votePost } from "@/services/voting.service";
...
/** Only up / down in the UI; counts are independent totals. */
type BinaryVote = "UPVOTE" | "DOWNVOTE";

function parseUserVote(v: FeedPost["userVote"]): VoteType | null {
  if (v === "UPVOTE" || v === "DOWNVOTE" || v === "NEUTRAL") return v;
  return null;
}

/**
 * Same rules as the API:
 * - click same as current → remove vote (toggle off)
 * - click different → remove old bucket, add new (e.g. up → down)
 * - no vote → add clicked bucket
 */
function optimisticVotePatch(post: FeedPost, clicked: BinaryVote): Partial<FeedPost> {
  const prev = parseUserVote(post.userVote);
  let up = Math.max(0, n(post.upvoats));
  let down = Math.max(0, n(post.downvoats));
  let neu = Math.max(0, n(post.neutralvoats));

  if (prev === clicked) {
    if (clicked === "UPVOTE") up -= 1;
    else down -= 1;
    return {
      upvoats: Math.max(0, up),
      downvoats: Math.max(0, down),
      neutralvoats: neu,
      userVote: null,
    };
  }
  ...
  if (prev === "UPVOTE") up -= 1;
  else if (prev === "DOWNVOTE") down -= 1;
  else if (prev === "NEUTRAL") neu -= 1;
...
export function PostCard({ post, onPatch }: Props) {
  ...
  const up = Math.max(0, n(post.upvoats));
  const down = Math.max(0, n(post.downvoats));
  const commentsCount = n(post.comment_count);
  const cur = parseUserVote(post.userVote);
  ...
  async function applyVote(clicked: BinaryVote) {
    ...
    onPatch(post.id, optimisticVotePatch(post, clicked));
    try {
      await votePost(post.id, clicked);
    } catch (e) {
